...
<%
    ...
    try
    {
      ...
      ResultSet rs=st.executeQuery("select count(*) from c where username='"+username+"'");
      while(rs.next())
      {
          int count=rs.getInt(1);
          if(count==0)
          {
              int i=st.executeUpdate("insert into user values('"+fname+"','"+lname+"','"+email+"','"+mobile+"','"+gender+"','"+address+"','"+username+"','"+password+"','Waiting')");
              if(i>0)
              {
                  %>
